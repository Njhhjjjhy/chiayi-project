...
The **forest** is the dark inner space, bounded by the pathway's inner partition along three walls (front, window, back) and additional partitions along the entrance-wall side that seal it from the entrance area.
...
The other door (whichever of D1 or D2 is *not* the first door) is outside the pathway route. The pathway partition does not extend past the first door toward the other door.
...
6. Place placeholder fact cards on the inner partition wall (the forest-side surface of the pathway). **Stop and flag** to ask Riaan for: total card count, per-segment distribution, minimum spacing, and whether to keep the existing card geometry from the old `Partition.jsx` (size, dimensions, mount style) or use a different geometry. Do not pick numbers or default to the existing geometry without confirmation.
...
**Lighting (pathway must be visibly lit at `DEFAULT_TIMELINE_T = 0.78`):**

The pathway is the educational entry. It must be visibly lit at the default darkness timeline position. **Stop and flag** to ask Riaan for the lighting spec before placing any lights. Do not pick values. The values needed:

- Number of point lights and spacing along the pathway.
- Light colour (hex).
- Light intensity.
- Light distance.
- Light height (y).
...
- `Partition.jsx` exists in the canonical room components folder identified in Step 0. The file at `src/components/proposals/Partition.jsx` no longer exists.
- The pathway is mounted in `Scene.jsx` unconditionally.
- Three pathway partitions wrap front, window, and back-walls in that order, terminating at the first door (whichever of D1 or D2 Riaan confirmed).
- Back-wall partition has an opening at the first door's x-position, with width equal to the pathway width. No opening near the other door.
...
- `grep -rn "mirror" <new Partition.jsx path>` returns zero matches.
- `grep -rn "side" <new Partition.jsx path>` returns no matches related to the old `side` prop (only matches related to actual geometry like `side: THREE.DoubleSide` are acceptable).
- `npm run build` exits 0.
...
1. Full JSX tree of `Scene.jsx` showing the curtain and pathway mounted unconditionally inside the canonical room.
2. Final geometry of all three pathway segments: which wall each runs along, start point, end point, length, orientation, width.
3. D1 and D2 positions and which constants define them. Confirm which one is the first door (used as the pathway exit) and confirm the other is not part of the pathway route.
...
- The HVAC plenum corner on the window-wall is not represented in the geometry, or the curtain or pathway would clip into it.
- `dimensions.js` has no obvious place to add door-position constants.
- Existing geometry in the canonical room conflicts with where the pathway needs to go.
