// Slice 6 verification: seating boxes and overhead spotlights.
// Four standard presets plus two diagnostics.
//
// Output:
//   baselines/<timestamp>-slice6-after-<preset>.png
//
// Diagnostics:
//   close-range-seating   — material read at ~1.5 m from zone 1 pair.
//   seating-spotlight-pool — zone 2 with overhead spot's floor pool in
//                            frame, captured in experience mode so the
//                            pool reads against a dim room.
...
const VIEWS = [
  ...
  { view: 'close-range-seating',   label: 'close-range-seating',   gridOff: false, experience: false },
...
async function capture({ view, label, gridOff, experience }) {
  ...
  const outPath = join(outDir, `${ts}-slice6-after-${label}.png`)
