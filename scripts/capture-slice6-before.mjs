// Before-baselines for slice 6 (seating boxes and overhead spotlights).
// Four standard presets. Topdown shot suppresses gridHelper via
// ?grid=off so the floor plane reads cleanly.
...
async function capture({ view, label, gridOff }) {
  ...
  const url = `http://localhost:5173/fireflies?view=${view}${gridParam}`
  await page.goto(url, { waitUntil: 'networkidle0', timeout: 60000 })
  await page.waitForSelector('canvas', { timeout: 15000 })
  await new Promise((r) => setTimeout(r, 5000))
  const outPath = join(outDir, `${ts}-slice6-before-${label}.png`)
