// Before-baselines for slice 7 (sculptural ceiling + 1,760 LED
// redistribution). Four standard presets. Topdown suppresses
// gridHelper via ?grid=off so the panel layout reads cleanly.
//
// The seated-POV-up diagnostic view requires a FirefliesPage edit
// (new view name handling). Deferred — Step 0 is read-only.
...
async function capture({ view, label, gridOff }) {
  ...
  const outPath = join(outDir, `${ts}-slice7-before-${label}.png`)
